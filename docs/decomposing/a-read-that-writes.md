# Decomposing a read that writes — the write nobody can find
...
That is a real file from a real repository, named `...Read`, calling `create!`. 376 of these
...
The call matrix **allows** a read to reach a record — that is what a read is for — so
`CallGraph` is silent by design. It governs which kinds talk, never what they say.

And the cost is not tidiness. **A read opens no transaction, because a read needs none.** The
generated `Read` deliberately has none. So the write runs outside any transaction the canon
knows about: a caller sequencing two reads has no rollback for the second, and every name on
...
`ReadsWriteNothing` names each site.
...
|---|---|
| `find_or_create_by`, `first_or_create` | a **write**; the caller decides when to create |
| a read that also touches, counts, caches, or stamps | a read, with the write moved to its caller |
| a write with a read stapled to the front | a write that answers what it wrote |
...
def call
  ...
  user.touch(:last_seen_at)          # <- the write, in a read, in the middle
...
thing that happened to be convenient here; it becomes its own write, called by whatever is
actually responsible for the session.

**Check:** `ReadsWriteNothing` is silent, and the read's name still describes everything it does.

---

## 3. Sequence with a workflow, not a read
...
**Do not put the sequencing in a write.** A write calling a write has either nested a
transaction or silently widened one, and nobody decided which — `CallGraph` refuses it, and
`a-write-is-one-transaction` is why.
...
single read just had no way to express it — but now it is your problem in writing.
...
`Shipshape/WritesProveIdempotence` requires the test to say *how* — a sentence beginning
...
`ReadsWriteNothing` needs a write rooted in a **record constant it can resolve**.
...
in the presentation layer — but a read holding one in a local is legal and invisible.

```sh
grep -rn "\.save!\|\.update!\|\.touch\|\.destroy" app/reads app/services
```

**Check:** the grep returns nothing in a read tree that the cop did not already name.

---

## What this leaves you

**Call sites that read like what they do.** A name ending in `Read` reads, a name that is a
...
`find_or_create_by` read into two operations moves a decision to the call site, and every
