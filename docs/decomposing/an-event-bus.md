...
## 4. Asynchronous subscribers become deferred writes, one job each
...
`call_later` defers **one write, one transaction, one job**
([`deferral-is-one-write`](../laws/deferral-is-one-write.md)), so a retry re-runs exactly
one transaction. A subscriber list delivered as one job retries all of them, including the ones
that already succeeded.

**Check:** each deferred step is its own write, and
`Shipshape/WritesProveIdempotence` is silent — a queue retries, and it will.
...
outbound call: an `io_write`, on the call path, with a timeout
