...
so: it is named for the question that rebuilds it, and every deed that invalidates it names it
...
## 3. Denormalisation — delete the column, and the answer is a question
...
**The fix is not a better column.** It is the existence of rows, read by a question:
...
A saved question answer, kept because the question is genuinely too slow. **Legitimate, and the last
thing to reach for.**

**Name it `{question_name}_cache_record`.** `ListOrderTotals` gets `ListOrderTotalsCacheRecord`.
The name is not decoration: it says which question owns the row, so the pairing is derivable in
...
**It always has a matching Question, and that question rebuilds it.** Not "the source still exists
somewhere" — one question, named by the record, which returns exactly what the record holds.
Rebuilding is running it. A cache record with no matching question is not a cache; it is a
denormalisation that has been given a reassuring name.

**Invalidation happens in the deeds that wrote the rows the question reads.** Not a TTL, not a
callback, not a subscriber — the deed that changed the underlying data is the thing that
knows the cache is now wrong, and it says so in the same transaction as the write
([`a-deed-is-one-transaction`](../laws/a-deed-is-one-transaction.md)). There is no window
in which the rows have moved and the cache has not.

**This is only enumerable because the question is a class.** A question declares its reads in one
file, so its source tables can be listed; from those, the deeds that write them can be listed; and
...
([an unbounded read](an-unbounded-read.md)), or the question doing its work in the database rather
than in Ruby. Slowness is the motivation, not the permission.

**Check:** drop the table in a staging environment and rebuild it by running the question. Nothing
...
**Check:** every deed in the list above either invalidates the cache or is one you can say
does not touch what the question reads.
...
**The cache's pairing is checkable, and that is a cop that could exist.** `{question_name}_cache_record`
makes the question's name derivable from the record's, so a guard could fail a `*CacheRecord` with
no matching `Question` class — the shape this procedure calls a denormalisation with a reassuring
name. It has not been written. What no guard can add is the other half: that every deed
writing the question's source tables invalidates it, which needs to know what the question reads, and
