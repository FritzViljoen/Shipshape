...
```ruby
# the permission is the class name of the write that does the thing
EditStory.call(actor: actor, story_id: id, title: title)

# the setting is a row, read by the read that builds the shape
plan.allows_images?
```
...
permission already exists the moment the write does:
...
plan.allows_images?   # a column on plans, read by the read that builds the shape
```
...
the write's own test names the outcome:

```sh
grep -rn "too_old" app/writes spec test
```
