...
```sh
grep -rn "\.find(\|\.find_by(\|\.find_by_id\|\.where(id:" app/reads app/writes app/io_reads app/io_writes
```
...
one. Ownership stops being a check somebody remembers and becomes the shape of the read.
