...
Declare the tree as `legacy_question` or `legacy_deed` — a legacy door is exactly a wrapper
around old code — or as `question`/`deed` if you intend to convert rather than wrap.
...
And the cops are not the only free list. **Every `transaction do` in the service is a deed
...
legacy file that was not inferred. Extracted into a `Deed`, that block's own `transaction`
...
judgement — a question answers shapes, a deed answers a `Result`. Do this before anything
else, because every later decision depends on which side a piece is on.

**Check:** `shipshape check` — the count falls, and no `CallGraph` offence appears (a question
...
A question answers a shape or an array of them, detached from the database. If a piece currently
...
**Check:** the `Question` base class refuses anything that is not a `Shape`, at the door.
