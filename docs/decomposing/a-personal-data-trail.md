...
and the anonymisation is a deed, called before the delete, named for what it does.
...
in a deed, and a workflow sequences them:
...
[a question that writes](a-question-that-writes.md) and [inline IO](inline-io.md) describe, with a
worse failure.
...
**Check:** for each `io_deed`, you can say whether personal data is in the payload, and if
