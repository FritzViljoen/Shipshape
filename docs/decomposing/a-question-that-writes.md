# Decomposing a question that writes — the write nobody can find
...
The call matrix **allows** a question to reach a record — that is what a question is for — so
`CallGraph` is silent by design. It governs which kinds talk, never what they say.

And the cost is not tidiness. **A question opens no transaction, because a read needs none.** The
generated `Question` deliberately has none. So the write runs outside any transaction the canon
knows about: a caller sequencing two questions has no rollback for the second, and every name on
...
`QuestionsNeverWrite` names each site.
...
|---|---|
| `find_or_create_by`, `first_or_create` | a **deed**; the caller decides when to create |
| a read that also touches, counts, caches, or stamps | a question, with the write moved to its caller |
| a write with a read stapled to the front | a deed that answers what it wrote |
...
def call
  ...
  user.touch(:last_seen_at)          # <- the write, in a question, in the middle
...
thing that happened to be convenient here; it becomes its own deed, called by whatever is
actually responsible for the session.

**Check:** `QuestionsNeverWrite` is silent, and the question's name still describes everything it does.

---

## 3. Sequence with a workflow, not a question
...
**Do not put the sequencing in a deed.** Two deeds always done together are one deed — a deed
calling a deed has either nested a transaction or silently widened one, and nobody decided
which — `CallGraph` refuses it, and `a-deed-is-one-transaction` is why.
...
single question just had no way to express it — but now it is your problem in writing.
...
`Shipshape/DeedsProveIdempotence` requires the test to say *how* — a sentence beginning
...
`QuestionsNeverWrite` needs a write rooted in a **record constant it can resolve**.
...
in the presentation layer — but a question holding one in a local is legal and invisible.

```sh
grep -rn "\.save!\|\.update!\|\.touch\|\.destroy" app/questions app/services
```

**Check:** the grep returns nothing in a question tree that the cop did not already name.

---

## What this leaves you

**Call sites that read like what they do.** A name ending in `Question` reads, a name that is a
...
`find_or_create_by` question into two operations moves a decision to the call site, and every
