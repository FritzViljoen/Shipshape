...
| **Find** | `@story = Story.find(params[:id])` | into a question, or into the operation, which takes the id |
| **Authorise** | `redirect_to root unless @story.editable_by?(current_user)` | disappears — the operation's base class checks |
| **Decide** | `redirect_to onboarding_path unless current_user.onboarded?` | into a deed; comes back as a failure code |
...
shape built by a question.

**Check:** `CallGraph` and `NoUnparsedLookup` are silent on the controller.

---

## 4. The deciding filter is a deed, and the redirect is its failure code
...
**Moving it up into a filter and moving it into a deed are not the same move**, and the
