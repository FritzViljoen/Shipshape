...
**Check:** `TypedArguments` is silent on the callee, and `QuestionsNeverWrite` has not appeared at
...
one answers a `Result` for a deed, shapes for a question. **The call site is where that lands,
