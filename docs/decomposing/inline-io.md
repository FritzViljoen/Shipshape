...
class SettleInvoice < Workflow
  def call
    charged = ChargeCard.call(actor: actor, invoice_id: @id)   # IoDeed, no transaction
...
- **in a deed** — a database transaction is open across the network round trip, including
...
- **in a question** — worse, because there is no transaction to blame and nothing about a read
  invites review.
...
**The `IoDeed` does the call and nothing else.** Not the decision about what to do with the
...
An `io_deed` may not reach a record. The matrix refuses it, and the reason is this exact
...
That state was always reachable. Written as one deed it was reachable *and unnameable* —
...
**Nothing here shows the retry is safe.** Splitting one deed into a workflow makes the
...
and that combination is strictly worse than the single deed it replaced, because the
