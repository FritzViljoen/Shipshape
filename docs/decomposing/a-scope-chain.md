# Decomposing a scope chain — a read whose name was never written
...
A chain is not a read object in disguise; it *is* a read, spelled out at the call site
...
## 2. Name the answer, not the read
...
**A scope used by exactly one read is not shared, and never was.** A scope used by six is the
interesting case: either it is one rule with six callers, in which case it becomes one read
...
A chain carrying `includes(:tags)` is encoding what some template touches, in the read, at a
distance from the template. Move it and it is not lost — it is **fixed**: the shape the read
...
**Check:** the read's own test asserts the number of database calls, and the number does not
...
`Read#call` refuses anything that is not a shape, so the template stops receiving records —
which is the whole of [a form that fails](a-form-that-fails.md), and it is a bigger job than
naming the read was. **Do it per template, not per read**: a read answering shapes whose
...
Every site that spelled the chain by hand now calls the read. That is
...
The other one to expect: **a read that now returns fewer rows because a scope was applied
