...
## It is a write turned inside out

**The write is already written — it is in the action, with its parts the wrong way round.**
...
| In the action | What it is in the write |
|---|---|
...
| `@story = Story.find_by(...)` | a read, or the write's own load |
| each `render` / `redirect_to` arm | one outcome the result has to be able to carry |

**The last row is the load-bearing one.** An action's response arms are the write's result
vocabulary, and counting them says what the write must be able to answer with before a line
of it is written. Four arms means four outcomes; a write that can answer two of them has
...
touches, so [a god record](a-god-record.md) often has to come first or the extracted write
...
**And if the action contains a `transaction do`, that block is the write** — its contents
...
**Check:** you can name the write or read before writing it.
...
The find is a **read**. The permission is the operation's own business — with authorisation
...
contract. One shape, built by the read, is what the view should be handed —
