...
| [a scope chain](a-scope-chain.md) | `Story.where(...).joins(...).order(...)` — a question nobody named |
...
| [an unbounded read](an-unbounded-read.md) | a question with no answer to "how many?" — no page, no limit, no cursor |
...
| [a question that writes](a-question-that-writes.md) | a class named `...Query` calling `create!` |
...
invalid record, a question that must answer shapes, and `form_with model:`. If that cost is not
...
**A deed is one transaction** ([`a-deed-is-one-transaction`](../laws/a-deed-is-one-transaction.md)),
...
one act*, and that decision is a deed waiting to be named.
...
**Take the whole block, not the method around it.** The block's contents are the deed's
...
|---|---|---|
| a **model** method | 98 | a god record doing the work — [a god record](a-god-record.md) first, and the block is the deed that comes out |
| a **service** method | 89 | a deed, and the rest of the method is its caller — [a service](a-service.md) |
| a **controller** action | 60 | the action's whole job — [a fat controller](a-fat-controller.md) |
| a **job** or elsewhere | 60 | usually a deed with a doorbell attached |

A transaction wrapping **two** unrelated acts is two deeds, and the warning below applies.
...
one transaction into two deeds means the middle state becomes reachable: the first
...
work, leave the transaction alone**: one deed doing two acts is a smell, and two deeds
...
exactly the ambiguity `a-deed-is-one-transaction` refuses when it forbids a deed calling
a deed: two deeds always done together are one deed. An inner block is usually a deed that
was already extracted in someone's head.

**Check:** every block on the list is either a deed now, or has a sentence next to it
saying why it is not. Once it is a deed (or a workflow of them), `Shipshape/OperationsOpenNoTransaction`
...
[a god record](a-god-record.md) comes first or the extracted deed wraps the same god
object.
