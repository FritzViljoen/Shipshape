...
|---|---|
| **Normalisation** — strip, downcase, default a field | into the **deed** that builds the record, or a database default's honest replacement |
| **Derivation** — set a column from other columns | a **question**, computed when asked, not stored |
| **A second effect** — send mail, enqueue a job, touch another record | its **own deed**, called by name |
...
that fires at a deed which never mentioned it is a callback with a table in front of it, and
...
**The deed reads the policy; the policy does not fire the deed.** That is the whole
