...
|---|---|---|
| Service objects | Is this | `Write` / `Read`. Sister calls refused before the matrix is read |
| Result objects | Is this | write answers `Result`. Read answers shapes, no envelope |
| Value objects | Is this | `Shape`. Takes records out of the view |
| Read objects | Is this, one disagreement | answer **shapes, not relations**. Caller cannot chain |
...
| Repository pattern | Refused | record maps rows. Read reads with the framework, answers shapes |
...
| Hexagonal / clean | Refused | a record is the mapping. Adapters are `io_write`, `io_read` |
