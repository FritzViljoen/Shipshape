...
|---|---|---|
| Service objects | Is this | `Deed` / `Question`. Sister calls refused before the matrix is read |
| Result objects | Is this | deed answers `Result`. Question answers shapes, no envelope |
...
| Repository pattern | Refused | record maps rows. Question reads with the framework, answers shapes |
...
| Hexagonal / clean | Refused | a record is the mapping. Adapters are `io_deed`, `io_question` |
