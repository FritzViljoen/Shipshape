...
having and refuse the half that costs.** CQRS is the clearest case — its own initials are
Command and Query; this canon calls the same split `Deed` and `Question`. What it is for is that a
...
`Deed` and `Question`, enforced by `Shipshape/QuestionsNeverWrite` — over one table, one record
...
| **Repository pattern** — hide ActiveRecord behind an interface | You are fighting the framework, and you lose relations, scopes and eager loading to a mapping layer you now maintain for ever | **Refused.** **What you get is the boundary without the mapping layer.** A record maps rows and a question reads them *using* the framework — relations and eager loading all still work, inside the question. What does not escape is the record itself: a question answers shapes. That is the separation a repository wanted, with no interface to write and nothing to keep in step |
...
| Pattern | Where it goes wrong | What you get here |
|---|---|---|
| **CQRS** — separate read and write models | Two models, two paths, double the surface area, to solve an asymmetry a typical application does not have | **Is this, at one tenth the cost.** The split is at the **operation**, not the model — the acronym's own C and Q, called `Deed` and `Question` here: `Deed` writes, `Question` reads, `QuestionsNeverWrite` enforces it. **What you get is the separation on one table, one record class and one migration path** — no second model, no projections, and no read path that can fall behind the write path, because there is only one |
...
| **Hexagonal / clean architecture** — ports, adapters, entities apart from AR | A mapping layer maintained for ever against a database you never actually swap | **Refused.** **What you get is the same isolation with nothing to maintain**: a record *is* the mapping, a shape is the domain object, and the only adapters are `io_deed` and `io_question` — operations on the call graph rather than a layer wrapped around the application |
...
without base-class permissions, or returning shapes from questions without a call graph — buys the
