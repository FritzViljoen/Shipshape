# `a-question-never-writes` — A question never writes, and the call graph cannot tell you otherwise

A question reaches a record, shapes what it found, and answers. It never writes. A write is a
deed, with its own name, its own permission and its own transaction.

**The call graph allows the call and cannot see the message.** A question reaching a record is
exactly what a question is for, so `record` is in the question's matrix row and always will be.
What the matrix governs is *which kinds talk*; what it cannot govern is *what they say*. A
question calling `PersonRecord.create!` is a permitted call carrying a forbidden message, and
that is the one gap the matrix leaves open by design.

**A question opens no transaction, because a read needs none.** The generated `Question`
deliberately has none — a transaction wrapped around a read holds a connection for nothing.
So a write from inside a question runs outside any transaction this canon knows about. A caller
sequencing two questions has no rollback for the second, and every name on the path says nothing
happened. That is the whole cost: not untidiness, but a write nobody can undo and nobody can
find.

**It is also how a question stops being one.** The name says read, the call site reads as a read,
and the caller has no reason to think about ordering, retries or idempotence. A deed
announces all three by being a deed.

## What a question answers, and how a caller asks
...
question answers one or many. Asking a **local** instead is refused: nothing at the condition then
...
- **Guard:** `Shipshape/PresenceIsNotRedefined`, over the shape tree. Fails `present?`, `blank?`
  ...
  whether the question found anything. The other half is
...
- **Guard:** `Shipshape/QuestionsNeverWrite`, over `question`, `io_question` and `legacy_question`. Fails a
...
- **Guard's limit:** the write must be rooted in a **record constant this configuration
  ...
  **And it does not check the other direction:** a deed that only reads is not reported,
  because "this class performs no write" is a claim about absence that no read of the source
  can make — the write may be one call away. That a read-only deed should have been a question
  stays a judgement.
