# `a-read-writes-nothing` — A read performs no write, and the call graph cannot tell you otherwise

A read reaches a record, shapes what it found, and answers. It never writes — writing is its
own kind, with its own name, its own permission and its own transaction.

**The call graph allows the call and cannot see the message.** A read reaching a record is
exactly what a read is for, so `record` is in the read's matrix row and always will be.
What the matrix governs is *which kinds talk*; what it cannot govern is *what they say*. A
read calling `PersonRecord.create!` is a permitted call carrying a forbidden message, and
that is the one gap the matrix leaves open by design.

**A read opens no transaction, because a read needs none.** The generated `Read`
deliberately has none — a transaction wrapped around a read holds a connection for nothing.
So a write from inside a read runs outside any transaction this canon knows about. A caller
sequencing two reads has no rollback for the second, and every name on the path says nothing
happened. That is the whole cost: not untidiness, but a write nobody can undo and nobody can
find.

**It is also how a read stops being one.** The name says read, the call site reads as a read,
and the caller has no reason to think about ordering, retries or idempotence. A write
announces all three by being a write.

## What a read answers, and how a caller asks
...
read answers one or many. Asking a **local** instead is refused: nothing at the condition then
...
- **Guard:** `Shipshape/PresenceIsNotRedefined`, over the shape tree. Fails `present?`, `blank?`
  ...
  whether the read found anything. The other half is
...
- **Guard:** `Shipshape/ReadsWriteNothing`, over `read`, `io_read` and `legacy_read`. Fails a
...
- **Guard's limit:** the write must be rooted in a **record constant this configuration
  ...
  **And it does not check the other direction:** a write that only reads is not reported,
  because "this class performs no write" is a claim about absence that no read of the source
  can make — the write may be one call away. That a read-only write should have been a read
  stays a judgement.
