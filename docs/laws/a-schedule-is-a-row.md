...
arrives, an actor is established, one deed runs. The only difference from a request is that
...
supposed to be free to change.** Storing a deed's class name in a column would make renaming
...
it would for a browser, so the chain is schedule → request → controller → deed, and every
...
which is the difference between this and a `config/schedule.rb` where the deed was never
