# `deferral-is-one-write` — Work is deferred one write at a time, or not at all

`SettleInvoice.call_later(actor: actor, invoice_id: 1)` puts **one write** on a queue: one
transaction, one job. A retry therefore re-runs exactly one transaction, which is the only
grain at which a retry is safe.

## Not a workflow, and not a read

A workflow spans several transactions. One job holding three of them retries steps that
already committed, so deferral belongs to the steps and never to the sequence. A read
answers shapes to a caller, and a deferred read answers nothing to nobody.
...
so the write owns repetition itself. Deferral is what turns that from tidy into
load-bearing — a queue retries on a timeout, a deploy, a worker restart, and a write that
...
- **Guard's limit:** **a failure is dropped, and whether that is right depends on the failure
  ...
  `Shape#to_h` is the caller's answer. Nothing checks that a deferred write is idempotent
  in fact; `Shipshape/WritesProveIdempotence` checks only that somebody wrote down how.
