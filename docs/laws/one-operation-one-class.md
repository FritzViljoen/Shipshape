...
it is the one judgement the rest of the model leans on: when deciding what a deed or
question should cover, ask who is allowed to do it. If one actor may do half of what it does
...
in a deed, which answers one message. Nothing in the module separates those. What decides
...
- **Guard:** the generated `operations_expose_nothing_test.rb`, installed into the
  ...
  **The exact form of this law, and the only one that cannot be fooled.** The cops read
  ...
  `SettleInvoice.public_instance_methods - Deed.public_instance_methods` is precisely what
