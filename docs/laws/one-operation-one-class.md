...
- **Guard:** `Shipshape/OneOperationOneClass`, over classes of an operation kind. Fails a
  second public method, a public method not named `call`, a positional parameter, a
  positional Hash default, `**rest`, and `(...)`.
- **Guard's limit:** it cannot tell whether the one method does one thing. A two-hundred
  ...
  cover them. It also cannot see a public method added at runtime.
