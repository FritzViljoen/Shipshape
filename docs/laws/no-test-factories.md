...
beside the writes and obeying none of their rules. No permission is checked, no arguments are
...
write that confirms one.

## The state a factory builds may be unreachable

This is the argument, and the rest are consequences of it.

A factory sets columns. A write enforces the rules that decide which combinations of columns
...
The inverse is worse and quieter. If a write *cannot* reach a state it should, the test that
...
nothing to call until the writes are there.
...
`Write`, `Read`, `LegacyWrite` and `LegacyRead` carry a second entry point — every kind
that reaches a record. `IoWrite` and `IoRead` do not: they touch no record, so there is no
...
Building state through writes is slower than an `INSERT`: every setup pays typed arguments
...
- **Guard's limit:** **a record written directly is not matched, and that is deliberate.**
  ...
  **The message assumes `test_call` exists, and a non-auth install has none at all.**
  ...
  false)` renders a `Write`, `Read`, `LegacyWrite` and `LegacyRead` with no second entry
  ...
  **`Workflow` carries no `test_call` either, on any install — the one omission that is not
  `IoWrite`/`IoRead`'s.** Those two are absent from the enumeration above because they touch
