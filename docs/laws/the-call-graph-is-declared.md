...
`io_deed` is a deed whose store belongs to somebody else — so a deed may not call
one.

**The reason is the transaction, and it is the only one that matters.** A deed is exactly
...
transaction open just as long. So neither a deed nor a question does IO — the external call
...
**An `io_deed` touches no record**, for the same reason: a failed remote call then leaves
...
**And it reads in its own world, not in ours.** `io_deed → io_question` mirrors
`deed → question` — a write may read first, and fetching a token before posting is that
shape. What it may not do is reach a `question`: an external write pulling from the local store
...
There are two rather than one so the return shape survives the crossing: a legacy question
answers with shapes, a legacy deed answers with a Result. A single door would have to
...
only happens to wrap something old. A deed may read through the reading door exactly as
it reads through a question, and may not call the writing door at all, exactly as it may not
call a deed.
...
A workflow may sequence deeds and questions and nothing else, so an unwrapped legacy
...
**Workflow, deed and question are all *operations*** — a class with one public method, as
...
Everything the MVC model used to hold has been split into workflows, deeds, questions and
...
no configuration can allow one. `Sisters` declares the groups — a legacy deed is a
deed that wraps something old, so the two are one kind for this purpose.
...
- **A deed is exactly one transaction. A workflow is several.** That is the sharpest
  form of the rule and the reason for it. Two deeds always done together are one deed —
  a deed that calls a deed has either nested a transaction or silently widened one, and
  nobody decided which — whereas a workflow crossing transactions is *obliged* to make
  each step idempotent and each intermediate state legal. Sequencing writes is the
  workflow's job because a workflow is the thing that has accepted that bill.
- **A question owns its read entirely**: it reads every table it needs
  and builds every part it returns. Two questions always composed together are one
  question's reads — a question that calls a question hides the second behind the first,
  invisible to whoever asked, and that is the shape an N+1 arrives in — whether the second
  read fetches a list or fetches one customer.

  **Where two questions want the same sub-shape, that is the signal it is not a part but a
  peer.** Promote it, give it its own question, and let whoever wanted both do the combining —
  a workflow or a deed may call two questions, and deciding to combine two reads is a
...
be one deed wrapping a second in a transaction. It becomes a workflow, and a workflow
...
- **Guard:** `shipshape check`'s coupling ratchet — `Shipshape::Coupling`,
  ...
  **One resolver is not one base dir.** A nested `.rubocop.yml` shifts `Shipshape/CallGraph`'s
  ...
  glob is not that case.** `Deed` declared at `app/deeds/deed.rb` resolves to that
