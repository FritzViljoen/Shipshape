...
Talking to anything across a network is what `io_deed` and `io_question` are for. A deed,
a question, a workflow, a shape, a component, a record and a controller do not do it.

**The reason is the transaction, and it is the same reason a deed may not call a deed:
two deeds always done together are one deed.** A deed is exactly one transaction, opened by
the base class before `call` runs. An HTTP
...
**A question is the worse case, not the better one.** It has no transaction to blame, so nothing
...
idempotent and each intermediate state legal. Written as one deed they are one step that
...
refuses `deed → io_deed`, and that works only for IO an application has already filed
...
- **Guard:** `Shipshape/IoIsItsOwnKind`, over every kind except `io_deed`, `io_question` and
