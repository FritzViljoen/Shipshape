...
module included into a deed puts that module's public methods onto the deed from a file
the deed's own definition never mentions, so `Shipshape/MixinsAddNothingPublic` forces a
...
from the other side: a concern defining `anonymous_call` and included into forty deeds makes
...
`include Paying` does in a deed: it puts behaviour on the class from a file nobody reading
...
deed — it forces the mixin's methods private with `Shipshape/MixinsAddNothingPublic`, so the
module's behaviour is still there but nothing outside the deed can reach it through the seam
...
A deed with a private mixin still answers through one public method; the class's own
...
survives for a test is the one that survives for a deed with no public surface to protect
