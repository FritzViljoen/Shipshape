# `a-deed-runs-twice` — Every deed says what happens when it runs again

A deed's test names what makes a second run safe. Not that it is safe — that it was
...
```ruby
# refused — the caller has taken a decision that belongs to the deed
SettleInvoice.call(...) unless invoice.settled?
```

Which leaves the deed owning repetition, whether or not anybody noticed. Every deed in
...
A queue retries. A deed that double-applies turns one retry into two charges, and the retry
...
same conclusion [`a-question-that-writes`](../decomposing/a-question-that-writes.md) and
[`inline IO`](../decomposing/inline-io.md) reach: the index is not optional.

- **Principle:** `tell-dont-ask`
- **Guard:** `Shipshape/DeedsProveIdempotence`, over `deed`, `io_deed` and
  `legacy_deed`. Fails a deed whose test file does not carry the words `Idempotent:`.
...
- **Guard's limit:** **it checks that the claim was written, never that it is true.** A deed
  ...
  It finds tests by **file name**. A deed exercised only from a shared example, a request
  spec covering four deeds, or a differently-named file reads as untested — a false positive
  ...
  deed that is idempotent today and stops being so when a column is added.
