...
`SettleInvoice < Deed` is an operation. `AdminSettleInvoice < SettleInvoice` is not, and
...
`anonymous_call` and included into forty deeds makes forty public operations, none of
...
Two deeds always done together are one deed, which the matrix refuses — so the shared part is a
question, or the sequence is a workflow. **The call graph already had an answer for this**; the
...
- **Principle:** `nothing-is-hidden` governs — a guarantee decided two classes away is a
  ...
  collaborator half.
- **Guard:** the generated `deed.rb`, `question.rb`, `workflow.rb`, `io_deed.rb`,
  `io_question.rb`, `legacy_deed.rb` and `legacy_question.rb` — architecture. Each owns
...
- **Guard:** `Shipshape/OnlyTheDoorIsCalled` is the one that does not rely on visibility:
  ...
  **The depth rule is about this canon's base classes and no others.** A plain class in
  `app/questions/` resolves to an operation kind by path alone, and `ApplicationMailer` is
