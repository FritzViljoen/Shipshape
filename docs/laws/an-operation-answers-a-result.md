...
A command, a workflow and the legacy doors answer with a `Result`: `success(value)` or
`failure(:code)`. A query answers with shapes and no envelope, because a read that found
nothing found nothing — that is an answer, not a failure.
...
- **Guard:** the generated `command.rb`, `workflow.rb`, `io_command.rb`, `legacy_command.rb`
  ...
  anything but a Symbol, so an error sentence cannot be smuggled in as a code. Proven by
