...
A write, a workflow and the legacy doors answer with a `Result`: `success(value)`,
`failure(:code)`, or `failure(:code, value)` where the caller needs to know what was wrong. A
read answers with shapes and no envelope, because a read that found nothing found nothing —
...
- **Guard:** the generated `write.rb`, `workflow.rb`, `io_write.rb`, `legacy_write.rb`
...
- **Guard's limit:** it is a runtime assertion, so it fires when the path runs rather than when
  ...
  read, which answers shapes and has no envelope to assert.
