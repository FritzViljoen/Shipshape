...
- **Guard's limit:** it reads **migrations, not the live schema**. A column made nullable by
  ...
  **It also fires only on a table it can see is owned.** A table counts as claimed when a
  file under the `record` kind's own paths either assigns `self.table_name` to it in a
  literal string, or declares a class whose superclass is a listed record base class, in
  which case the guard guesses Rails' default: the pluralised, underscored, demodulised
  class name. It cannot see a table that no Record file mentions at all — hundreds of them
  in an adopting repository — and it never claims to have modelled what such a table means;
  it reports only what a column it inspected says. It also cannot see: an irregular plural
  (`Person` guessed as `persons`, not `people`); a table name computed rather than written as
  a literal; a subclass of another record rather than of the listed base classes, which
  claims nothing of its own even when Rails would derive one from it; or a record living
