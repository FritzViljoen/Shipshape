...
over 1,087 of code — 66%, and four of them were false. `io_write` described what happened
"after the transaction" it opens none of. `workflow` told views to call `permissions`, which
had been made private. `read` documented an audit entry a read has never written. `write`
