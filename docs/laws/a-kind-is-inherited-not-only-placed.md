...
`Foo` is `deed` to all forty-some cops this gem ships. `Shipshape/OperationsOpenNoTransaction`
...
coverage. It is the wrong one for every cop that reads `deed` or `record` and assumes the
...
`Deed` at `app/deeds/deed.rb` is a legitimate layout, and `class Deed` there
...
- **Principle:** `make-the-wrong-thing-impossible` — a kind's guarantees are supposed to be
  ...
  `deed` off a path alone and trusts the rest is the "write it down and hope" this
...
- **Guard's limit:** it resolves a superclass exactly as `Shipshape/CallGraph` and
  ...
  here too: `Class.new(Deed)`, a superclass assigned through a constant or produced by a
  ...
  `an-operation-is-a-leaf`'s guard uses — so `Vendor::Gubbins::Deed` passes as `Deed`,
  and a local `module Reports; class Deed; end` buys itself the exemption `Deed` itself
