...
**A record is never an argument.** Not into a deed, not into a shape, not into anything.
...
- **Guard:** `Shipshape/PresentationHoldsNoRecords`, over the `shape` and `view_component`
  trees. **A record is only allowed in a deed or a question**, and the matrix holds that
