# `a-deed-is-one-transaction` — One transaction, opened by the base class, however many writes it holds

**A deed may write as many rows to as many tables as one act needs** — an invoice and its
...
The unit is the **act**, not the row. A workflow is several deeds and therefore several
...
tidiness. Two deeds always done together are one deed — so a deed that calls another deed has
either nested a transaction or silently widened one, and nobody decided which. The second
deed's rollback semantics are now decided by whichever caller happens to be on the stack,
which is a different answer per call site and written nowhere.

**Four writes inside one deed are not a smell.** They either all happen or none do, which
is the whole point of the transaction being there. Four writes across two deeds called in
...
each subclass, the law would be true by convention: some deeds would open one, some would
...
**A question opens nothing.** A read needs no transaction, and one wrapped around a read holds a
connection for no reason. The generated `Question` therefore has no transaction at all — the word
appears in that file only in a comment about the door.

- **Principle:** `make-the-wrong-thing-impossible`
- **Guard:** the generated `deed.rb` and `legacy_deed.rb` — architecture rather than a
  ...
  deed that calls a deed, which is what would nest one.
  `Shipshape/OperationsOpenNoTransaction` refuses a `transaction` send written inside any
  operation kind — catching a second one opened directly in `deed` and `legacy_deed`,
  one opened where a workflow's steps should each own their own, one held open over the wire
  in `io_deed` and `io_question`, and one wrapped around a read that needs none in `question` and
  `legacy_question`. Proven by `generated_base_classes_test.rb`, a listed suite guard.
- **Guard's limit:** nothing counts the writes, and nothing should — the number is not the
  ...
  deed updating an invoice and archiving an unrelated report is two acts sharing a
