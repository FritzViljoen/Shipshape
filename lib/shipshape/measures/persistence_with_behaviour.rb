  ...
  module Measures
    ...
    class PersistenceWithBehaviour
      ...
      def proposal(findings)
        ...
        kind = finding.context[:writes] ? "Write" : "Read"
      ...
      def reasoning(finding, kind)
        return "It calls `#{finding.context[:write]}`, so it writes: a Write." if finding.context[:writes]

        "Nothing in it writes, so it is a Read. A method that only derives a value from " \
