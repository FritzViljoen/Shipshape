...
module Shipshape
  module Measures
    ...
    module Naming
      ...
      # says nothing about direction, and guessing Deed from silence gets laughed at.
      ...
      def kind_for(action, message: nil)
        return "Deed" if message && WRITES.include?(message.to_sym)
        return "Question" if message

        READS.include?(action.to_sym) ? "Question" : "Deed"
      end

      def path_for(name, kind)
        directory = kind == "Question" ? "questions" : "deeds"
