...
class CoverageTest < Minitest::Test
  LAYOUT = {
    "Shipshape/CallGraph" => {
      "Kinds" => { "deed" => ["app/deeds/**/*.rb"], "record" => ["app/records/**/*_record.rb"] },
      "BaseClasses" => { "deed" => ["Deed"], "record" => ["ApplicationRecord"] },
      "Matrix" => { "deed" => ["record"], "record" => [] },
    },
  }.freeze

  def test_it_counts_what_resolves_to_a_kind
    result = measure(
      "app/deeds/settle.rb" => "class Settle < Deed\nend\n",
  ...
  def test_vendored_and_generated_trees_are_not_the_denominator
    result = measure(
      ...
      "test/deeds/settle_test.rb" => "class SettleTest\nend\n",
