...
class ReportTest < Minitest::Test
  ...
  def test_it_proposes_a_shape_named_from_their_own_code
    ...
    assert_includes proposal, "class FindOrder < Read"
    assert_includes proposal, "app/reads/find_order.rb"
    assert_includes proposal, "@id = typed(id, Integer)",
      "A suggestion written in the reader's own nouns is one they can argue with; one written in `YourWrite` and `SomeModel` is a slide."
  end

  def test_a_proposal_with_no_inputs_has_no_empty_initializer
    proposal = row("Rules living on persistence").proposal

    assert_includes proposal, "class TotalOrder < Read"
  ...
  def test_the_message_decides_read_or_write_where_the_action_cannot
    assert_equal "Read", Shipshape::Measures::Naming.kind_for(:archive_order, message: :where)
    assert_equal "Write", Shipshape::Measures::Naming.kind_for(:index, message: :create!),
      "The message is better evidence than the action name: `User.where` is a read whatever the method around it is called, and guessing Write from silence is how a report gets laughed at."
  end
  ...
  def test_it_infers_write_or_read_from_what_the_method_does
  ...
  def test_the_proposal_names_the_evidence
    ...
    assert_includes proposal, "class SettleOrder < Write"
    assert_includes proposal, "app/writes/settle_order.rb"
    assert_includes proposal, "It calls `update!`, so it writes: a Write."
  end
  ...
  def test_it_shows_the_shape_it_is_asking_for
    ...
    # Record, shape, read, write, workflow — the order the rename makes possible, and
    # the order a reader can follow: the table lets go of the name, then the name is used.
    order = ["class InvoiceRecord", "class Invoice < Shape", "class FindInvoice < Read",
             "class SettleInvoice < Write", "class CloseTheMonth < Workflow"]
    ...
    # read -> read, the sister call this canon calls load-bearing.
    refute_includes text, "FindCustomer.call"
    assert_includes text, "A READ OWNS ITS ENTIRE READ"
  ...
  def test_a_method_that_reaches_the_database_cannot_move
    ...
    refute_includes labels.join, "#auto_settled?"
    refute_includes labels.join, "#settle!",
      "Not a matter of taste: a shape has no database, so a method needing one cannot exist on it. `auto_settled?` becomes a field the read fills, not a Read class of its own."
  end
  ...
  def test_a_mailer_is_an_outbound_write_not_an_entry_point
    ...
    assert_includes kinds.fetch("io_write").join, "app/mailers"
  ...
  def test_a_write_that_names_a_user_is_not_a_question
  ...
  def test_a_predicate_inside_an_operation_is_the_model_working
    assert_empty authorisation_in("class Settle < Write\n  def can_run?\n    true\n  end\nend\n",
                                  relative: "app/writes/settle.rb")
  end
