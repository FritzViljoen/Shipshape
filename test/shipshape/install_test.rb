...
class InstallTest < Minitest::Test
  ...
  # must be too. Watched to fail: putting the line above `<%- if auth -%>` in `deed.rb.tt`
  # reddens this, because the rendered `Deed` then describes a method it does not define.
  ...
  def test_it_writes_every_base_class
    in_app do |root|
      ...
      assert_path_exists File.join(root, "app/shipshape/deed.rb")
      assert_path_exists File.join(root, "app/shipshape/legacy_question.rb")
  ...
  def test_it_never_overwrites
    in_app do |root|
      ...
      target = File.join(root, "app/shipshape/deed.rb")
  ...
  def test_a_file_absent_is_written
    in_app do |root|
      ...
      assert_includes report[:written], "app/shipshape/deed.rb"
      assert_empty report[:skipped]
  ...
  def test_a_file_present_and_differing_is_reported_and_left_untouched
    in_app do |root|
      ...
      assert_includes report[:diverged], "app/shipshape/deed.rb"
      refute_includes report[:skipped], "app/shipshape/deed.rb"
      ...
      in_app { |fresh| install(fresh) && assert_equal(read_installed(fresh, "app/shipshape/deed.rb"), File.read(new_file)) }
    end
  end
  ...
  def test_a_new_file_no_longer_needed_is_reported_stale_and_left_alone
    in_app do |root|
      ...
      assert_includes report[:stale], "app/shipshape/deed.rb"
      assert_empty report[:diverged]
  ...
  def test_they_land_outside_the_governed_trees
    in_app do |root|
      ...
      refute_path_exists File.join(root, "app/deeds")
      refute_path_exists File.join(root, "app/questions"),
        "The base classes define the shapes; they are not instances of them. `Deed` is not a deed, so they sit outside the governed trees and no cop classifies them."
    end
  end
  ...
  def test_it_writes_every_class_the_default_config_names
    ...
    generated = %w[Workflow Deed Question IoQuestion IoDeed LegacyQuestion LegacyDeed Shape]
  ...
  def test_a_directory_may_be_chosen
    in_app do |root|
      ...
      assert_includes report[:written], "lib/shapes/deed.rb"
    end
  end
