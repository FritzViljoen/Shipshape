...
class EveryDoorChecksPermissionTest < Minitest::Test
  ...
  def test_a_door_that_lost_its_check_is_an_offence
    found = offences(GUTTED, cop_class: COP, path: "app/shipshape/write.rb", files: WITH_AUTH)

    assert_equal 1, found.length
    assert_includes found.first.message, "`write.rb` is a door and no longer checks a permission"
  end

  def test_the_offence_carries_the_reason_and_an_example
    message = offences(GUTTED, cop_class: COP, path: "app/shipshape/write.rb", files: WITH_AUTH).first.message
  ...
  def test_a_door_that_still_checks_is_the_shape
    assert_empty offences(<<~RUBY, cop_class: COP, path: "app/shipshape/write.rb", files: WITH_AUTH)
      class Write
        extend Permission
  ...
  def test_an_application_without_authorisation_is_left_alone
    assert_empty offences(GUTTED, cop_class: COP, path: "app/shipshape/write.rb"),
