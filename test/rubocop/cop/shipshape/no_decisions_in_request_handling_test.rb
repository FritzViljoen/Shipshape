...
class NoDecisionsInRequestHandlingTest < Minitest::Test
  ...
  def test_a_deed_may_branch_all_it_likes
    assert_empty offences(<<~RUBY, cop_class: COP, path: "app/deeds/cancel_booking.rb", other_cops: LAYOUT)
      class CancelBooking
        def call
  ...
  # `present?` is correct across all three answers a question may give: `nil` and `[]` are absent,
  # a shape and an array of shapes are present.
  def test_present_asks_whether_a_question_found_anything
    assert_empty check(<<~RUBY)
      class BookingsController
        def show
  ...
  # A question may answer nothing, so the caller has to ask whether it found any — and `present?`
  # is the one spelling. Bare truthiness and its cousins say the same thing in other words.
  def test_only_present_asks_whether_a_question_found_anything
  ...
  def test_a_deed_result_is_still_the_shape
    assert_empty check(<<~RUBY)
      class BookingsController
        def update
