...
class OnlyTheDoorIsCalledTest < Minitest::Test
  ...
  LAYOUT = {
    "Shipshape/CallGraph" => {
      ...
      "BaseClasses" => {
        "write" => ["Write"], "workflow" => ["Workflow"], "shape" => ["Shape"]
      },
      "Matrix" => {
        "request_handling" => %w[write workflow shape],
        "write" => ["shape"], "workflow" => ["write"], "shape" => []
      },
    },
  }.freeze

  DEFERRABLE = { "DeferrableKinds" => %w[write], "DeferredMessages" => %w[call_later] }.freeze
  ...
  def test_a_write_may_still_defer
  ...
  def test_an_operation_held_in_a_variable_is_the_stated_blind_spot
    assert_empty check("write = SettleInvoice\n    write.new(invoice_id: 1)")
  end
  ...
  def test_a_write_may_be_deferred
  ...
  def test_a_class_referring_to_itself_is_not_a_call_site
    assert_empty offences("class SettleInvoice < Write\n  def self.build\n    SettleInvoice.new\n  end\nend\n",
                          cop_class: COP, path: "app/writes/settle_invoice.rb",
