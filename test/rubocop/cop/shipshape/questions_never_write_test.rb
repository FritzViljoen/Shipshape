...
# call graph cannot hold this.** A question reaching a record is exactly what a question is for, so the
class QuestionsNeverWriteTest < Minitest::Test
  include CopRunner

  COP = RuboCop::Cop::Shipshape::QuestionsNeverWrite
  ...
  LAYOUT = {
    "Shipshape/CallGraph" => {
      ...
      "BaseClasses" => { "question" => ["Question"], "deed" => ["Deed"], "shape" => ["Shape"] },
      "Matrix" => { "question" => %w[shape record], "deed" => %w[shape record], "record" => [], "shape" => [] },
    },
  }.freeze
  ...
  QUESTION = "app/questions/list_people.rb"
  DEED = "app/deeds/create_person.rb"

  def test_a_write_from_a_question_is_refused
    found = check("PersonRecord.create!(name: \"x\")")

    assert_equal 1, found.length
    assert_includes found.first.message, "`PersonRecord.create!` is a write, and a question never writes"
  end

  def test_the_offence_carries_the_reason_and_an_example
    message = check("PersonRecord.destroy_all").first.message

    assert_includes message, "WHY: A question opens no transaction"
    assert_includes message, "The call graph cannot catch it"
    assert_includes message, "INSTEAD:"
    assert_includes message, "class CreatePerson < Deed"
  end
  ...
  def test_a_read_is_the_whole_point_of_a_question
    assert_empty check("PersonRecord.where(active: true).to_a")
    assert_empty check("PersonRecord.find(1)")
  end

  def test_a_deed_may_write
    assert_empty check("PersonRecord.create!(name: \"x\")", DEED)
  end
  ...
  def check(body, path = QUESTION)
    klass = path == QUESTION ? "class ListPeople < Question" : "class CreatePerson < Deed"
